...
export const QuantityForm: React.FC<QuantityFormProps> = ({ initialData }) => {
  ...
  const title = initialData ? "Edit Quantity" : "Create Quantity";
  const description = initialData ? "Edit a Quantity." : "Add a new Quantity";
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(
        `/api/${params.storeId}/quantites/${params.categoryId}`
      );
      router.refresh();
      router.push(`/${params.storeId}/quantites`);
